...
interface RootStore<R> extends Store<R> {
  rootGetters?: any;
}
...
class ModuleBuilderImpl<S, R = {}> implements ModuleBuilder<S, R> {
  ...
  module<M>(namespace: string, initialState?: M): ModuleBuilder<M, R> {
    ...
    if (!initialState) {
      ...
      if (!existingModule) {
        throw new Error(
          `There is no module named '${qualifiedNamespace}'.  If you meant to create a nested module, then provide initial-state as the second argument.'`
        );
      }
    ...
    if (existingModule) {
      throw new Error(
        `There is already a module named '${qualifiedNamespace}'.  If you meant to get the existing module, then provide no initialState argument.`
      );
    }
    const nestedBuilder = new ModuleBuilderImpl<M, R>(
      qualifiedNamespace,
      initialState
    );
  ...
  commit<P>(handler: MutationHandler<S, P>, name?: string) {
    ...
    return ((payload: P) =>
      this._store!.commit(namespacedKey, payload, useRootNamespace)) as any;
  }

  dispatch<P, T>(handler: ActionHandler<S, R, void, void>): () => Promise<void>;
  dispatch<P, T>(
    handler: ActionHandler<S, R, P, void>
  ): (payload: P) => Promise<void>;
  dispatch<P, T>(handler: ActionHandler<S, R, void, T>): () => Promise<T>;
  dispatch<P, T>(
    handler: ActionHandler<S, R, P, T>
  ): (payload: P) => Promise<T>;
  dispatch<P, T>(
    handler: ActionHandler<S, R, void, void>,
    name: string
  ): () => Promise<void>;
  dispatch<P, T>(
    handler: ActionHandler<S, R, P, void>,
  ...
  dispatch<P, T>(
    handler: ActionHandler<S, R, void, T>,
    name: string
  ): () => Promise<T>;
  dispatch<P, T>(
    handler: ActionHandler<S, R, P, T>,
  ...
  dispatch<P, T>(handler: any, name?: string): any {
    ...
    return (payload: P) =>
      this._store!.dispatch(namespacedKey, payload, useRootNamespace);
  }
...
function qualifyKey(
  ...
  name?: string
): { key: string; namespacedKey: string } {
  const key: string = name || handler.name;
  if (!key) {
    throw new Error(
      `Vuex handler functions must not be anonymous. Possible causes: fat-arrow functions, uglify.  To fix, pass a unique name as a second parameter after your callback.`
    );
  }
...
export interface StoreBuilder<R> {
  ...
  dispatch<P, T>(
    handler: ActionHandler<R, R, P, void>
  ): (payload: P) => Promise<void>;
  dispatch<P, T>(handler: ActionHandler<R, R, void, T>): () => Promise<T>;
  dispatch<P, T>(
    handler: ActionHandler<R, R, P, T>
  ): (payload: P) => Promise<T>;
  dispatch<P, T>(
    handler: ActionHandler<R, R, void, void>,
    name: string
  ): () => Promise<void>;
  dispatch<P, T>(
    handler: ActionHandler<R, R, P, void>,
  ...
  dispatch<P, T>(
    handler: ActionHandler<R, R, void, T>,
    name: string
  ): () => Promise<T>;
  dispatch<P, T>(
    handler: ActionHandler<R, R, P, T>,
...
const namedStoreBuilderMap: {
  [name: string]: StoreBuilderImpl<any>;
} = Object.create(null);
...
export function getStoreBuilder<R>(name?: string): StoreBuilder<R> {
  ...
  const builder =
    ...
    (namedStoreBuilderMap[name] = new StoreBuilderImpl<R>());
  return builder;
}
